...
def run(env, test_env, device, buffer):
    args = get_args()
    assert args.algo == "ddpg", "unexpected envoking with algorithm not set to be ddpg"
    assert env.continuous, "ddpg only deals with continuous action space"
    
    save_dir = os.path.join(os.getcwd(), args.save_dir, args.exp_name + "_seed_" + str(args.seed))
    os.makedirs(save_dir, exist_ok=True)

    test_env.eval()
    agent = DDPGAgent(args, env, device)
    ...
    for T in tqdm_bar:
        ...
        if T > args.random_act_steps:
            action = agent.act(obs, args.online_noise_scale)
        else:
