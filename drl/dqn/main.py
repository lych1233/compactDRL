...
def run(env, test_env, device, buffer):
    args = get_args()
    assert args.algo == "dqn", "unexpected envoking with algorithm not set to be dqn"
    assert env.discrete, "dqn only deals with continuous action space"
    
    save_dir = os.path.join(os.getcwd(), args.save_dir, args.exp_name + "_seed_" + str(args.seed))
    os.makedirs(save_dir, exist_ok=True)

    test_env.eval()
    agent = DQNAgent(args, env, device)
