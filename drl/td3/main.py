...
def run(env, test_env, device, buffer):
    args = get_args()
    assert args.algo == "td3", "unexpected envoking with algorithm not set to be td3"
    assert env.continuous, "td3 only deals with continuous action space"
    
    save_dir = os.path.join(os.getcwd(), args.save_dir, args.exp_name + "_seed_" + str(args.seed))
    os.makedirs(save_dir, exist_ok=True)

    test_env.eval()
    agent = TD3Agent(args, env, device)
